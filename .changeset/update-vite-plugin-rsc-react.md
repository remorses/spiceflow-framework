...
Fix `useId()` hydration mismatch in production builds caused by a missing toast renderer placeholder in the SSR render tree. The SSR tree structure now matches the browser tree exactly so React generates identical IDs during server rendering and client hydration.
